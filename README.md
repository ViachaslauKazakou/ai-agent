...
При старте, если `.aiagent/session.json` уже существует, агент отдельно
спрашивает, загружать ли его. Значение по умолчанию — `N`, поэтому без явного
подтверждения создаётся новая сессия, а старый файл будет заменён после первого
сохранения. Команда `/load` позволяет загрузить сессию позже.

Любой текст, не начинающийся с `/`, отправляется как пользовательский prompt.

Сессия сохраняется в `.aiagent/session.json`. Помимо сообщений, файл содержит
безопасный краткий список событий tools: имя, успешность и длительность.
Аргументы вызовов и содержимое результатов в audit log не сохраняются.
