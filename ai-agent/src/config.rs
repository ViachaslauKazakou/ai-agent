...
impl Config {
    ...
    pub fn load(cli: &Cli) -> Result<Self, AppError> {
        ...
        Self::from_sources(cli, &environment)
    }
    ...
    pub fn from_sources(
        ...
        let base_url = cli
            .base_url
            .clone()
            .or_else(|| environment.get("LITELLM_BASE_URL").cloned())
            .unwrap_or_else(|| DEFAULT_BASE_URL.to_owned());
        let api_key = environment.get("LITELLM_API_KEY").cloned();
        let model = cli
            .model
            .clone()
        ...
        let max_tool_rounds = match cli.max_tool_rounds {
            Some(value) => value,
